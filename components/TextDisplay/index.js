...
const TextDisplay = () => {
  ...
  */
  const inViewSentences = sentences.filter(
    (s, i) => i >= selectedAreaArr[0] && i <= selectedAreaArr[1]
  );
  ...
  */
  ...
  for (
    let i = absoluteSentenceIndexesThatIncludeSelectedTheme.length - 1;
    i >= 0;
    i--
  ) {
    ...
    const currentSentenceTextWithTheme =
      inViewSentences[
        absoluteSentenceIndexesThatIncludeSelectedTheme[i].relativeI
      ].text;
    const openingTagIndex = resText.indexOf(currentSentenceTextWithTheme);
    const closingTagIndex =
      openingTagIndex + currentSentenceTextWithTheme.length;
