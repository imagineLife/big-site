...
const Area = () => {
  const { sentences, selectedAreaArr, maxWordsPerSentence } =
  ...
  const optHoverLine =
    ...
    !showLine ? null : (
      <line
        ...
        y1={yScale(0)}
        y2={yScale(
          Math.max(...selectedSentences.map((d) => d.wordCount)) * 1.05
        )}
      />
    );
  ...
  */
  const hoverCircle =
    !hoverLine || offsetSentenceNumber < 0 || !showLine ? null : (
      <circle
        pointerEvents="none"
        r={8}
  ...
  return (
    <>
      ...
      >
        ...
        {/* Area Path */}
        <path d={pathD} fill="rgba(147,147,147,.25)" />
      ...
      <div className="explanation-wrapper">
        <p className="explanatory-text">
          {!curSentence &&
            'Hover over this area chart to highlight the sentence that was spoken at the specific point in time during the president’s address.  HOVER'}
